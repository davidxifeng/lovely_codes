...
static int lencrypt(lua_State *L) {
  size_t dlen, klen;
  unsigned char * data = (unsigned char *)luaL_checklstring(L, 1, &dlen);
  unsigned char * key = (unsigned char *)luaL_checklstring(L, 2, &klen);
  xxtea_long rlen;
  char * s = (char *)xxtea_encrypt(data, dlen, key, klen, &rlen);
  lua_pushlstring(L, s, rlen);
  free(s);
  return 1;
}
...
static int ldecrypt(lua_State *L) {
  ...
  unsigned char * key = (unsigned char *)luaL_checklstring(L, 2, &klen);
  xxtea_long rlen;
  char * s = (char *)xxtea_decrypt(data, dlen, key, klen, &rlen);
  lua_pushlstring(L, s, rlen);
  free(s);
  return 1;
}

static const luaL_Reg xxtea_libs[] = {
  {"encrypt", lencrypt},
  {"decrypt", ldecrypt},
  {NULL, NULL}
};
...
LUALIB_API int luaopen_xxtea(lua_State *L) {
  luaL_newlib(L, xxtea_libs);

  lua_pushliteral(L, "xxtea for cocos, created on Tue 16:43 Jun 14");
