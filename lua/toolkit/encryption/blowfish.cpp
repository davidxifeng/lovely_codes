...
static int ldecrypt(lua_State *L) {
  CBlowFish *bf = *(CBlowFish **)luaL_checkudata(L, 1, BF_MT);
  size_t klen;
  unsigned char * key = (unsigned char *)luaL_checklstring(L, 2, &klen);
  int mode = luaL_optinteger(L, 3, 0);
  bf->Decrypt(key, klen, mode);
...
static int lnew(lua_State *L) {
  size_t len;
  unsigned char * key = (unsigned char *)luaL_checklstring(L, 1, &len);
  CBlowFish ** pbf = (CBlowFish **)lua_newuserdata(L, sizeof(CBlowFish *));
  *pbf = new CBlowFish(key, len);
  if (!*pbf) {
    return 0;
  }
  luaL_setmetatable(L, BF_MT);
  return 1;
}

static const luaL_Reg blowfish_libs[] = {
  {"encrypt", lencrypt},
  {"decrypt", ldecrypt},
  {"reset",   lreset},
  {"__gc",    lgc},
...
LUALIB_API int luaopen_blowfish(lua_State *L) {

  if (luaL_newmetatable(L, BF_MT)) {
    luaL_setfuncs(L, blowfish_libs, 0);
    lua_pushvalue(L, -1);
  ...
  lua_pushliteral(L, "blowfish for cocos, created on Wed 18:51 Jun 15");
